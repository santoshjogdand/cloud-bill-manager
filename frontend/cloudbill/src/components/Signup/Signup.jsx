...
import OtpVerificationModal from "./otpVerificationModal.jsx"; // Import the OTP verification modal
...
const SignupModal = ({ onClose }) => {
  ...
  const validatePhone = (phone) => {
    const re = /^[0-9]{10}$/; // Adjust the regex for your phone number format
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validatePassword(formData.password)) {
      ...
      showPopup("Password must be at least 8 characters long, contain at least one uppercase letter, one lowercase letter, one number, and one special character.","error");
      setLoading(false);
      return;
    }

    try {
      const response = await API.post("register/initiate", formData);
      if (response.status === 200) {
        showPopup("OTP sent to your email. Please verify to complete registration.", "success");
